...
function createPendingTextMessage(text, recipientId = "") {
...
async function syncPendingMessages() {
  if (!state.user || state.syncing || !navigator.onLine) return;
    ...
    for (let index = 0; index < queue.length; index += 1) {
      ...
      try {
        const { message } = await api("/api/messages", {
          method: "POST",
          body: JSON.stringify({ text: item.text, recipientId: item.recipientId || "" })
        });
        if (state.messages.has(item.localId)) {
          state.messages.delete(item.localId);
          state.messages.set(message.id, message);
...
async function submitText(text) {
  ...
  try {
    ...
    if (!isOfflineError(error)) throw error;
    const message = createPendingTextMessage(text, currentRecipientId());
    const queue = pendingQueue();
    queue.push({ localId: message.id, text, recipientId: message.recipientId || "", conversationId: message.conversationId, createdAt: message.createdAt });
